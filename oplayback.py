...
def playback(transcript):
    ...
    try:
        ...
        print("Program execution interrupted by the user (ctrl-c)")
        return
...
if __name__ == "__main__":
    ...
    mqttclient = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, "Playback")
    connect_to_mqtt(mqttbroker, mqttport)

    with open(filename, 'r') as f:
        transcript_lines = f.readlines()
    transcript = [p.strip('\n') for p in transcript_lines]
